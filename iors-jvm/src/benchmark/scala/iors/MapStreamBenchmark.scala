 ...
 *
 *     jmh:run -i 10 -wi 10 -f 2 -t 1 iors.MapStreamBenchmark
 *
...
object MapStreamBenchmark {
  ...
  final case class Stream(value: Int, next: IoRs[Option[Stream]])
  val addOne = (x: Int) => x + 1

  def range(from: Int, until: Int): Option[Stream] =
    if (from < until)
      Some(Stream(from, IoRs(range(from + 1, until))))
    else
      None
  ...
  def sum(acc: Long)(box: Option[Stream]): IoRs[Long] =
    box match {
      case Some(Stream(value, next)) =>
